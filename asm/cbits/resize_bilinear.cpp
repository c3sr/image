...
#if 1
extern "C" void resize_hori(float *dst, float *src, uint64_t h, uint64_t dst_w,
                            uint64_t src_w) {
  ...
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < dst_w; j++) {
      ...
      if (x >= (src_w - 1)) {
        ...
        dst[i * dst_w + j] =
            (1.0f - dx) * src[i * src_w + x] + (dx)*src[i * src_w + x + 1];
      }
    }
  }
}

extern "C" void resize_vert(float *dst, float *src, uint64_t w, uint64_t dst_h,
                            uint64_t src_h) {
  ...
  for (int i = 0; i < w; i++) {
    for (int j = 0; j < dst_h; j++) {
      ...
      if (y >= (src_h - 1)) {
        dst[i * dst_h + j] = src[i * src_h + src_h - 1];
      } else {
        dst[i * dst_h + j] =
            (1.0f - dy) * src[i * src_h + y] + (dy)*src[i * src_h + y + 1];
      }
    }
  }
}
