...
func ResizeNearestNeighbor(inputImage types.Image, height int, width int) (types.Image, error) {
	return nativeResizeNearestNeighbor(inputImage, height, width)
}

func IResizeNearestNeighbor(targetPixels []uint8, srcPixels []uint8, targetWidth, targetHeight, srcWidth, srcHeight int) error {
	return resizeNearestNeighborNative(targetPixels, srcPixels, targetWidth, targetHeight, srcWidth, srcHeight)
}

func nativeResizeNearestNeighbor(in types.Image, targetHeight, targetWidth int) (types.Image, error) {
	...
	case *types.RGBImage:
		...
		err := resizeNearestNeighborNative(res.Pix, in.Pix, targetWidth, targetHeight, srcWidth, srcHeight)
		return res, err
	case *types.BGRImage:
...
func resizeNearestNeighborNative(targetPixels []uint8, srcPixels []uint8, targetWidth, targetHeight, srcWidth, srcHeight int) error {
	inputImage := toRGBAImage(srcPixels, srcWidth, srcHeight)
	resized := imaging.Resize(inputImage, targetWidth, targetHeight, imaging.NearestNeighbor)
